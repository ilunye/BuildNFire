...
public class NetBombTrigger : NetworkBehaviour
{
    ...
    void Start()
    {
        ...
        sum_Msg = GetComponent<Netsum_msg>();
    }
    ...
    public void rightBomb()
    {
        ...
        if (cannons[1].GetComponent<NetCannon>().idx == 10)
        {
    ...
    public void leftBomb()
    {
        ...
        if (cannons[0].GetComponent<NetCannon>().idx == 10)
        {
            GameObject brokenCannon = Instantiate(Resources.Load("prefabs/broken_cannon") as GameObject);
    ...
    private void ExplosionLeft(float force, Vector3 position, float radius)
    {
        ...
        if(mode == 0){
            Destroy(player_one);
            player_two.GetComponent<NetCharacter>().CmdPlay("WIN00");
        }else{
            Destroy(player_one);
            Destroy(player_two);
        }
    ...
    private void ExplosionRight(float force, Vector3 position, float radius)
    {
        ...
        if(mode == 0){
            Destroy(player_two);
            player_one.GetComponent<NetCharacter>().CmdPlay("WIN00");
        }else{
            player_one.GetComponent<NetCharacter>().CmdPlay("WIN00");
            player_two.GetComponent<NetCharacter>().CmdPlay("WIN00");
        }

...
public class NetBuffHandler : NetworkBehaviour
{
    ...
    public void CmdBuffDestroy(GameObject obj)
    {
    ...
    public void GetCollisionBuff(BuffInfo buffInfo, GameObject CollObj) //得到碰撞物体的buff信息
    {
        ...
        if (buffInfo != null)
        {
            ...
            {
                if (gameObject.name == "animal_people_wolf_1")
                    StartCoroutine(ChangeColorCoroutine(GameObject.Find("animal_people_wolf_2")));
                else
                    StartCoroutine(ChangeColorCoroutine(GameObject.Find("animal_people_wolf_1")));
            }
            CmdBuffDestroy(CollObj);
        }
    }
    ...
    IEnumerator ChangeColorCoroutine(GameObject target1)
    {
        ...
        target1.GetComponent<NetCharacter>().CmdSetColor(new Color(65 / 255f, 105 / 255f, 225 / 255f));
        yield return new WaitForSeconds(7f);
        target1.GetComponent<NetCharacter>().CmdSetColor(Color.white);
